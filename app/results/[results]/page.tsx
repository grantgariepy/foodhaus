...
const fetchSearchResults = async (results: string) => {
  // console.log(results);
  const res = await fetch(
    `https://www.themealdb.com/api/json/v2/${mealDBAPI}/search.php?s=${results}`,
...
export default async function Results({ params: { results } }: PageProps) {
  const searchResults = await fetchSearchResults(results);
        ...
        <div className='max-w-screen-2xl px-4 md:px-8 mx-auto'>
          ...
          {searchResults.meals == null ? (
            ...
            <div className='grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-4 md:gap-x-6 gap-y-8'>
              {/* product - start */}
              {searchResults.meals.map((meal: any, index: number) => (
                <div key={index}>
                  {/* <Link href=>
            </Link> */}
                  <Link
